...
import { Avatar,  Typography, styled,Card, Box, Badge, AvatarGroup, useMediaQuery } from "@mui/material";
...
function UserChat({ chat, user }: { chat: Chat; user: User }) {
 ...
 function UserBadge({recipientName,isOnline}:{recipientName:string,isOnline:boolean}) {
  return (
    <>
    <StyledBadge
    overlap="circular"
  ...
  >
    <Avatar
      {...stringAvatar(recipientName || "Anonyms")}
    />
  </StyledBadge>
  </>
  )
